...
def BinaryToDecimal():
    ...
    for i in range(len(binary)):  #to reprompt the user if he/she inputs a non-binary number
        if int(binary[i]) > 1 or int(binary[i]) < 0:
            binary = list(input("Enter the binary number: "))
    power = 0
    output = 0
    while binary:
...
def HexadecimalToDecimal():
    ...
    for i in range(len(number)): #ensure user inputs an uppercase letter
        if number[i].islower():
            number = list(input("Enter the Hexadecimal(must be uppercase): ")) 
    power = 0
    output = 0
    while number:
